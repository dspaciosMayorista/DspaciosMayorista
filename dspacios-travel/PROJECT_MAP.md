...
| Dashboard administrativo | `app/(dashboard)/dashboard/page.tsx` (Server Component) + shell (`layout.tsx`, `DesktopSidebar.tsx`, `Topbar.tsx`, `TenantSwitcher.tsx` con `@base-ui/react/select`), tokens propios `--dash-*` en `DashboardShell.module.css`. KPI reales (barra solo con numerador/denominador real): Contratos, Cupos, Cartera por moneda, Pagos por vencer, Conciliaciones/DIAN/Retenciones (roles contables). Lógica pura y testeable en `lib/dashboard/metricas.ts`; solo `ventas.estado` `confirmado`/`activo` cuenta como venta efectiva (nunca `pendiente`/`cancelado`). Meta general mensual persistida por tenant+periodo+moneda: tabla `meta_ventas_mensual` (migración 185) + editor `MetaVentasConfig.tsx` (`/dashboard/configuracion`, roles `superadmin`/`gerencia`/`administracion`) — nunca la suma de `asesores.meta_mensual` ni el cálculo dinámico de Punto de equilibrio. Agregados pesados resueltos en base vía 6 funciones `SECURITY INVOKER` (migración 186, `fn_dashboard_*`, `EXECUTE` solo `authenticated`/`service_role`) en vez de descargar filas completas; `cupos_por_bloqueo` con `security_invoker = true`. Utilidad genérica anti-"Max Rows": `lib/supabase/fetchAllPaginado.ts`. Pruebas: `pruebas/dashboardShellWiring.test.ts`, `dashboardMetricas.test.ts`, `dashboardMetricasAgregadas.test.ts`, `fetchAllPaginado.test.ts`. Ver `DECISIONS.md` ADL-026 | Autenticada (roles internos) |
