...
| **Condiciones de pago por componente** | [`condiciones-pago-componente.md`](./condiciones-pago-componente.md) | Migración 164 (cotización manual): condición de pago por hotel/vuelo/servicio, snapshot congelado (`cotizacion_condiciones`), pagos previos idempotentes (`registrar_pago_previo`), conversión atómica 1 cotización → 1 contrato, restricciones no-reembolsable/no-endosable, overrides append-only (Commit 6), candados de inmutabilidad autoritativos, y el guard de espejo Docker (`test_164_espejo.sh`, corregido en Commit 7). **+ migraciones 165/166 (PR #282, "Rama B"):** congela condiciones REALES de catálogo también en los 3 caminos que crean contrato directo (tarifario/reservar, programas, carrito) sin pasar por cotización manual, y corrige el bypass de `eliminar_contrato()` para contratos con condiciones congeladas. Las tres migraciones ya aplicadas en Supabase real. |
