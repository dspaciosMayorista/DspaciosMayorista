-- ───────────────────────────────────────────────────────────────────────────
-- 158 · VUELOS — modalidad_emision: CIERRE del rename individual → serie
--
-- ⚠️ RENUMERADA de 157 a 158 (rama `vuelos-empaquetados-editor-contrato`):
-- en producción ya corrieron 155 y 156, pero esta migración (entonces
-- numerada 157) seguía SIN ejecutarse — se le pudo cambiar el número sin
-- ningún riesgo. El número 157 quedó libre para una migración NUEVA y
-- totalmente distinta (editor operativo de vuelos del contrato — ver
-- `20260601000157_editor_vuelos_contrato.sql`), aditiva e independiente de
-- este cierre. Contenido de este archivo sin cambios de fondo, solo el
-- número (y las referencias a sí mismo/su rollback).
--
...
--
-- ROLLBACK PROBADO: `supabase/scripts/rollback_158_modalidad_emision_serie_cierre.sql`
...
comment on column public.bloqueos_vuelo.modalidad_emision is
  ...
  'renombró a "serie"; cerrado en la 158). Obligatoria en registros nuevos (validada '
  ...
  '(migración 156) cuando se muestra fusionada en Control Vuelos. Migración 158.';
...
comment on function public.actualizar_control_bloqueo(bigint, text, text, text, text) is
  ...
  'Modalidad válida: serie/grupo (cerrado en la migración 158; individual/serie/grupo durante '
  'la transición de la 155).';

-- Repetido explícitamente (152, 155, 158) — nunca se asume que sobrevive
...
begin
  ...
  if v_remanentes > 0 then
    raise exception '158 FALLÓ: % filas siguen con modalidad_emision=''individual'' después del cierre — el UPDATE no se aplicó a todas.', v_remanentes;
  end if;
  ...
  if v_remanentes > 0 then
    raise exception '158 FALLÓ: % filas tienen un modalidad_emision fuera de serie/grupo/null tras el cierre — revisar antes de continuar.', v_remanentes;
  end if;
end $$;
