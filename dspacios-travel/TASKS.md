...
### 1. Rediseno del Dashboard administrativo

- [ ] Auditar el Dashboard real (rutas `/dashboard/**`) contra la referencia visual de Stitch.
- [ ] Conservar el logo, el nombre D'Spacios Travel y el tratamiento de marca vigente.
- [ ] Mantener intacta la logica funcional, permisos (`lib/roles.ts`, `LECTURA_MODULO`), consultas y operaciones existentes.
- [ ] Redefinir estructura y sistema visual sin tocar precios, reservas, datos ni motores de calculo.
