...
export type PasajeroFila = {
  ...
  estado: string;
  esInfante?: boolean;
  /** Solo en filas de infante: `id` NUMÉRICO real de `contrato_pasajeros` (para poder editarlo — ver InfanteVueloForm). `id` (arriba) es la clave de fila sintética "infante-N", no sirve para el RPC. */
    ...
    <div>
      ...
      <div className="overflow-x-auto rounded-xl border border-gray-200 bg-white">
        <table className="w-full min-w-[1100px] text-sm">
          ...
          <tbody>
            {vis.length === 0 ? (
              <tr><td colSpan={11} className="px-3 py-10 text-center text-gray-400">Sin pasajeros para esta búsqueda.</td></tr>
            ) : vis.map((p) => (
              <Fragment key={p.id}>
                  ...
                  <td className="px-3 py-2 text-xs">
                    {p.sillaId != null && p.bloqueoId != null && p.tipoDoc && p.numeroDoc && (
                      <InfanteVueloForm
                        ...
                        sillaResponsableId={p.sillaId}
                        fechaIdaBloqueo={p.fechaIda}
                        modo="crear"
                      />
                    )}
                  </td>
                ...
                {(infantesPorPadre.get(p.id) ?? []).map((inf) => (
                  <tr key={inf.id} className="border-t border-gray-50 bg-gray-50/60">
                    <td colSpan={11} className="px-3 py-1.5 pl-8 text-xs">
                      <span className="inline-flex flex-wrap items-center gap-1.5 text-gray-600">
                        ...
                        {inf.infanteId != null && p.sillaId != null && p.bloqueoId != null && inf.tipoDoc && inf.numeroDoc && inf.fechaNacimientoInfante && (
                          <InfanteVueloForm
                            ...
                            inicial={{
                              id: inf.infanteId,
                              nombreCompleto: inf.nombres,
                              tipoDoc: inf.tipoDoc,
                              numeroDoc: inf.numeroDoc,
                              fechaNacimiento: inf.fechaNacimientoInfante,
                            }}
                          />
                        )}
