...
import { enviarEmail, componerHtml, type EmailConfig } from "@/lib/crm/email";
...
// Núcleo: envía un correo a una lista y registra la campaña.
async function enviarA(
  ...
  const html = componerHtml(cuerpoHtml, cfg.firma_html);
  ...
  await sb.from("crm_campanas").insert({
    asunto, cuerpo_html: cuerpoHtml, categoria, tipo,
...
export async function enviarCampana(input: { asunto: string; cuerpoHtml: string; categoria: string }): Promise<EnvioCampanaResult> {
  ...
  if (!input.cuerpoHtml.trim()) return { ok: false, error: "El cuerpo del correo está vacío." };
  ...
  return enviarA(sb, dest, input.asunto, input.cuerpoHtml, input.categoria || "todos", "campana");
}
...
export async function enviarFelicitacionCumple(input: { asunto: string; cuerpoHtml: string; soloHoy: boolean }): Promise<EnvioCampanaResult> {
  const sb = await createClient();
  if (!(await gate(sb))) return { ok: false, error: "Solo administración/gerencia." };
  if (!input.asunto.trim() || !input.cuerpoHtml.trim()) return { ok: false, error: "Asunto y cuerpo son obligatorios." };
  ...
  return enviarA(sb, dest, input.asunto, input.cuerpoHtml, "cumpleaños", "cumpleanos");
}
