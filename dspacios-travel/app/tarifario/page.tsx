...
export default async function TarifarioPublicoPage() {
  ...
  if (hotelIds.length) {
    const { data: hs } = await sb.from("hoteles").select("id, estrellas, clasificacion, descripcion, ubicacion, video_url, pax_min, pax_max, edad_nino_min, edad_nino_max, edad_infante_min, edad_infante_max, infante_cargo_neto, infante_cargo_desc, infante_nota, nino_nota, adults_only").in("id", hotelIds);
    for (const h of hs ?? []) {
      infoPorHotel[h.id] = { estrellas: h.estrellas, clasificacion: h.clasificacion, descripcion: h.descripcion, ubicacion: h.ubicacion, video_url: h.video_url, ninoMin: h.edad_nino_min, ninoMax: h.edad_nino_max, infMin: h.edad_infante_min, infMax: h.edad_infante_max, infanteCargo: (Number(h.infante_cargo_neto) || 0) > 0, infanteCargoDesc: h.infante_cargo_desc, infanteNota: h.infante_nota, ninoNota: h.nino_nota, adultsOnly: h.adults_only ?? false };
